...
class AESCipher(BlockCipher):
    ...
    def inv_substitute_bytes(self, state: list[list[int]]) -> list[list[int]]:
        for row in range(4):
            for col in range(4):
                state[row][col] = self.INV_S_BOX[state[row][col] >> 4][state[row][col] & 0x0F]
        return state
    ...
    def inv_mix_columns(self, state: list[list[int]]) -> list[list[int]]:
        for col in range(4):
            a = [state[row][col] for row in range(4)]
            state[0][col] = self.galois_field_mul(a[0], 0x0e) ^ self.galois_field_mul(a[1], 0x0b) ^ self.galois_field_mul(a[2], 0x0d) ^ self.galois_field_mul(a[3], 0x09)
            state[1][col] = self.galois_field_mul(a[0], 0x09) ^ self.galois_field_mul(a[1], 0x0e) ^ self.galois_field_mul(a[2], 0x0b) ^ self.galois_field_mul(a[3], 0x0d)
            state[2][col] = self.galois_field_mul(a[0], 0x0d) ^ self.galois_field_mul(a[1], 0x09) ^ self.galois_field_mul(a[2], 0x0e) ^ self.galois_field_mul(a[3], 0x0b)
            state[3][col] = self.galois_field_mul(a[0], 0x0b) ^ self.galois_field_mul(a[1], 0x0d) ^ self.galois_field_mul(a[2], 0x09) ^ self.galois_field_mul(a[3], 0x0e)
        return state
    ...
    def matrix_to_matrix_decrypt(self, state: list[list[int]]) -> list[list[int]]:
        # Inverse of last round, without mix column
        state = self.add_round_key(state, 10)
        state = self.inv_shift_rows(state)
        state = self.inv_substitute_bytes(state)
        # Inverse of nine rounds of process
        for round in range(9, 0, -1):
            state = self.add_round_key(state, round)
            state = self.inv_mix_columns(state)
            state = self.inv_shift_rows(state)
            state = self.inv_substitute_bytes(state)
        # Add encryption key, the "addition" here is defined as bitwise xor
        state = self.add_round_key(state, 0)
        return state

    def encrypt(self, plaintext: str) -> str:
        """
        The AES-128 encryption process
        Input: plaintext string with length of 16 (16 bytes)
        Output: hexadecimal ciphertext string with length of 32 (16 bytes)
        """
    ...
    def decrypt(self, ciphertext: str) -> str:
        ...
        Input: hexadecimal ciphertext string with length of 32 (16 bytes)
        Output: plaintext string with length of 16 (16 bytes)
        """
        assert len(ciphertext) == 32, "Ciphertext must be 16 bytes long."
        # print(f"ciphertext_hex = {ciphertext}")
        ciphertext_bytes = self._hex_to_bytes(ciphertext)
        # print(f"ciphertext_bytes: {ciphertext_bytes}")
        ciphertext_matrix = self._bytes_to_matrix(ciphertext_bytes)
        # print(f"ciphertext_matrix: {ciphertext_matrix}")
        plaintext_matrix = self.matrix_to_matrix_decrypt(ciphertext_matrix)
        # print(f"plaintext_matrix: {plaintext_matrix}")
        plaintext_bytes = self._matrix_to_bytes(plaintext_matrix)
        # print(f"plaintext_bytes: {plaintext_bytes}")
    ...
    def _bytes_to_hex(self, byte_list: list[int]) -> str:
